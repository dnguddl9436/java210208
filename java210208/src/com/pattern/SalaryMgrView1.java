 ...
 * 
 * 누가 누구를 인스턴스화 해야 하는가?
 * :A안에 B를 인스턴스화 해야하나? 아님 B안에 A를 인스턴스화 해야하나?
...
public class SalaryMgrView1 {
	   //선언부
	   SalaryMgrLogic1 sml = null;
	   SalaryMgrHandler1 smh = null;
	   JFrame				jf_sal		= null;
	   ...
	   public SalaryMgrView1() {
		  sml = new SalaryMgrLogic1(this);
		  smh = new SalaryMgrHandler1(sml);
	      ...
	      for(int i=0;i<jbtns_label.length;i++) {
	         ...
	         jbtns[i].addActionListener(smh);//this는 현재 활성화된 객체 주소번지 SalaryMgr
	      }
	      initDisplay();
	   }
