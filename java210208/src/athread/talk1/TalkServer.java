 ...
 * 이 스레드에서 소켓 서버 정보를 관리하는 관리하는 것이 경합이 일어날 수 있고 그에 따라 충돌이나 이상이
 * 발생 가능하므로 별도의 스레드를 구현하고 그 run메소드 안에서 안전하게 소켓들이 생성되고 관리될 수
 * 있도록 한다.
 * 
 */
public class TalkServer extends JFrame implements Runnable{
	...
	public void run() {
		...
		try {
			...
			//예외 발생에 대한 메시지들을 stack영역에 관리하는데 이것들을 출력해주는 메소드입니다.
